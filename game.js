    ...
    for (let bx = 0; bx < GRID_SIZE; bx++) {
      for (let bz = 0; bz < GRID_SIZE; bz++) {
        ...
        if (!isPlaza) {
          ...
          const center = new THREE.Vector3();
          box.getCenter(center);
          // Use the mesh's ACTUAL computed bounding-box center here, not the
          // (cx, cz) grid coordinate we positioned it at. Those are only the
          // same if the source model's own origin sits exactly at its visual
          // center — which real downloaded building models frequently don't
          // (same class of bug as the car's off-center pivot). Assuming they
          // matched was placing invisible static colliders in the wrong
          // spot — including, apparently, sometimes right on top of where
          // the player's car spawns, which is exactly why it looked "locked"
          // with nothing visibly blocking it.
          physics.addProp({ x: center.x, y: center.y, z: center.z }, { x: size.x / 2, y: size.y / 2, z: size.z / 2 }, 0);
