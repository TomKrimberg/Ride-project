...
export function createGame(AppState, InputState, physics, ui, audio, persist) {
  ...
  async function buildCity() {
    ...
    const [buildingsGltf, treesGltf, signGltf, lightGltf] = await Promise.all([
      ...
      for (let bz = 0; bz < GRID_SIZE; bz++) {
        ...
        if (!isPlaza) {
          ...
          for (let t = 0; t < 2; t++) {
            ...
            let tree;
            if (treesGltf) {
              tree = cloneGltf(treesGltf).scene;
        ...
        if (signGltf && Math.random() > 0.7) {
          const sign = cloneGltf(signGltf).scene;
  ...
  async function spawnNPC(loopPoints) {
    ...
    if (gltf) {
      ...
      mesh = type.kind === 'human' ? fallbackHumanoid(type.color) : fallbackQuadruped(type.color);
    }
    enableShadows(mesh);
    scene.add(mesh);
    const start = loopPoints[0];
    const entity = physics.addNPC(start, loopPoints, type.kind === 'human' ? 1.1 + Math.random() * 0.6 : 1.6 + Math.random() * 0.8);
  ...
  async function setActiveCar(carId) {
    ...
    if (gltf) {
      carGroup = cloneGltf(gltf).scene;
      const box = new THREE.Box3().setFromObject(carGroup);
      const size = new THREE.Vector3();
      box.getSize(size);
      const scale = 3.6 / Math.max(size.z, 0.1);
      carGroup.scale.setScalar(scale);
    } else {
      carGroup = fallbackCar(def.color);
    }
    ...
    physics.createCarBody(stats, { x: 0, y: 1.2, z: 0 });
    return carGroup;
  }
  ...
  function onNPCHit(npc, relVel) {
    ...
    const damage = Math.min(8, 2 + relVel * 0.4);
  ...
  function onImpact(relVel) {
    if (currentMode === 'menu') return;
    const damage = Math.min(18, relVel * 1.1);
    AppState.health = Math.max(0, AppState.health - damage);
  ...
  function startRun(mode) {
    currentMode = mode;
    paused = false;
    physics.resetCar({ x: 0, y: 1.2, z: 0 });
  ...
  function quitToMenu() {
    currentMode = 'menu';
    audio.stopEngine();
    InputState.steer = 0; InputState.throttle = 0; InputState.brake = 0; InputState.drift = false;
  }
